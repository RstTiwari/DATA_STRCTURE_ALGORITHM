...
def SJF_PRMEETIVE(processes: list):
    ...
    while completed < n:
        available_processes = [
            ...
            and remaining_burst_time[i] > 0
        ]
        if available_processes:
            shortest_job = min(
                available_processes, key=lambda x: remaining_burst_time[x]
            )
            ...
            if remaining_burst_time[shortest_job] == 0:
                ...
                turn_around_time[processes[shortest_job]["process_id"]] = (
                    current_time - processes[shortest_job]["arrival_time"]
                )
                waiting_time[processes[shortest_job]["process_id"]] = (
                    turn_around_time[processes[shortest_job]["process_id"]]
    ...
    print(
        f"{'Process':<10} {"Arrival Time":<15}  {"Burst Time":<15} {"Completion Time":<15} {"turn Around time":<15} {"Waiting Time":<15}"
    )
    for process in processes:
        print(
...
def priority_schduling_preemtive(processes):
    ...
    while completed < n:
        available_process = [
            i for i in range(n) if processes[i]["arrival_time"] <= current_time
        ]


processes2 = [
    {"process_id": "P2", "arrival_time": 0, "burst_time": 8, "time_quantum": 3},
    {"process_id": "P3", "arrival_time": 2, "burst_time": 7, "time_quantum": 4},
    {"process_id": "P4", "arrival_time": 3, "burst_time": 3, "time_quantum": 4},
    {"process_id": "P5", "arrival_time": 4, "burst_time": 4, "time_quantum": 4},
    {"process_id": "P1", "arrival_time": 0, "burst_time": 6, "time_quantum": 7},
]


# the most popular algorithm
def round_robin_scheduling(processes: list):
    ...
    while completed < n:
        ...
        if available_process:
            current_process_idx = available_process.pop(0)
            current_process = processes[current_process_idx]
            TQ = processes[current_process_idx]["time_quantum"]
            BT = remaining_burst_time[current_process_idx]
            if BT <= TQ:
                current_time += BT
                completion_time[current_process["process_id"]]= current_time
                remaining_burst_time[current_process_idx] = 0
                completed += 1
            else:
                current_time += TQ
                remaining_burst_time[current_process_idx] -= TQ

            if remaining_burst_time[current_process_idx] == 0:
                turnaround_time[current_process["process_id"]] = (
                    current_time - processes[current_process_idx]["arrival_time"]
                )
                waiting_time[current_process["process_id"]] = (
                    turnaround_time[current_process["process_id"]]
                    - processes[current_process_idx]["burst_time"]
                )
        else:
            current_time += 1
    ...
    for process in processes:
        print(
            f"{process['process_id']:<10}  {process["arrival_time"]:<15}  {process["burst_time"]:<15} {completion_time[process["process_id"]]:<15} {turnaround_time[process['process_id']]:<15} {waiting_time[process['process_id']]:<15}"
    )
...
if __name__ == "__main__":
    ...
    round_robin_scheduling(processes2)
